...
如果bean 的名称为 factoryBean 的格式，但它却不是 FactoryBean 的话就要抛出异常。如果它不是FactoryBean 的话直接返回就可以。如果是FactoryBean 则通过 getObjectFromFactoryBean 获取到 bean 的实例。获取 bean 的实例需要 bean 定义，因为要为 bean 的属性赋值。
