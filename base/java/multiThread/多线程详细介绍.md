...
4. 实现Callable 或者 runnable 接口，使用FutureTask包装，交给Thread执行。
...
![](https://pic4.zhimg.com/80/v2-6fdc939e35ec801c31ea515e80a88367_hd.jpg)

线程在给定时间点只能处于一种状态。 这些状态是虚拟机状态，不反映任何操作系统线程状态。NEW和RUNNABLE状态很好理解，那BLOCKED状态实在什么情况下产生的呢？
...
1. 在多个线程执行同一个对象的同步方法的时候。那些没有获取到监视器，希望进入方法执行的线程，就会处于BLOCKED状态。synchronized 代码块对一个 lock 对象上锁，也会造成阻塞。或者ReenTrantLock（使用了阻塞原语 LockSupport）。
...
notify():唤醒正在**此对象监视器上**等待的单个线程。 如果任何线程正在等待此对象，则选择其中一个线程被唤醒。 选择是任意的，由具体实现决定。 线程通过调用其中一个wait方法等待对象的监视器。
...
5. java.util.concurrent 包下面的 Semaphore、CountDownLatch、CyclicBarrier、Exchanger、phaser
